...
# ******************************************************************************
# * @brief Sum up all the size of all the directories with a size smaller of maxSize
# ******************************************************************************
...
# ******************************************************************************
# * @brief Find the directories with a size greater than minSize
# ******************************************************************************
def findBigDirectory (node, nodeCandidate, minSize=0):
    
    if(node.attributes == "dir"):
        for x in node.children.values():
            if (x.attributes == "dir"):
                nodeCandidate = findBigDirectory(x, nodeCandidate, minSize)
    
        if(node.size >= minSize and node.size<=nodeCandidate.size):
            nodeCandidate = node
...
if __name__ == '__main__':
    ...
    try:
        ...
        needSpace = 30000000
        toBeDeleted = needSpace - unusedSpace
        print("totalSpace:" + str(totalSpace), flush=True)
        print("usedSpace:" + str(usedSpace), flush=True)
        print("unusedSpace:" + str(unusedSpace), flush=True)
        print("needSpace:" + str(needSpace), flush=True)
        print("toBeDeleted:" + str(toBeDeleted), flush=True)     
        
        nodeCandidate = root
        nodeCandidate = findBigDirectory(root, nodeCandidate, toBeDeleted)
        print("Directory to be deleted:" + str(nodeCandidate.name) + " (" + str(nodeCandidate.size) + ")", flush=True)
